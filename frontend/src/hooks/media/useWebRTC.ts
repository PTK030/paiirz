...
export function useWebRTC(socket: Socket | null, room: string | null): UseWebRTCReturn {
  ...
  const createPeerConnection = useCallback(
    (onRemoteStream: (stream: MediaStream) => void): RTCPeerConnection => {
      ...
      pc.ontrack = (event) => {
        ...
        if (incomingStream) {
          // Replace or add tracks on the existing remoteMediaStream so the
          // video element's srcObject reference stays valid across renegotiations.
          incomingStream.getTracks().forEach((track) => {
            const existing = remoteMediaStream.getTracks().find((t) => t.kind === track.kind);
            if (existing) remoteMediaStream.removeTrack(existing);
      ...
      // connectionState covers cases that iceConnectionState misses (e.g. DTLS failure).
   ...
   * renegotiation needed) and notifies the remote peer via the existing
   * `webrtc_signal` channel so their UI can reflect the updated state.
   */
  const toggleMic = useCallback(() => {
    ...
    if (audioTrack) {
      ...
      sendSignal({ type: "mic-mute", muted: !audioTrack.enabled });
    }
  }, [sendSignal]);
  ...
  const toggleCamera = useCallback(async () => {
    ...
    if (!isVideoMuted) {
      ...
      // Notify partner that camera is off
      sendSignal({ type: "video-mute", muted: true });
    } else {
      // Turn camera ON
      try {
        ...
        setIsVideoMuted(false);
        // Notify partner that camera is back on
        sendSignal({ type: "video-mute", muted: false });
  ...
  useEffect(() => {
    ...
    const handleSignal = async ({
      ...
      if (type === "mic-mute") {
        setIsRemoteMicMuted(signal.muted ?? false);
        return;
      }

      if (type === "video-mute") {
        setIsRemoteVideoMuted(signal.muted ?? false);
        return;
      }
