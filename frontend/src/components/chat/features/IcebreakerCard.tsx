...
export const IcebreakerCard: React.FC<IcebreakerCardProps> = ({
  ...
  // `votes` is keyed by socket id, not by role ("me"/"partner"), so the
  // partner's entry has to be found by exclusion - there are only ever two
  // participants in a room, so "the other key" is always the partner.
  ...
  if (icebreaker.status === "proposed") {
    // The backend seeds `accepted_users` with only the game's creator at
    // creation time (see `ws_games.py`), so "have I already accepted" is an
    // exact proxy for "did I propose this game" - there's no separate
  ...
  if (isThisOrThat) {
    ...
    // "Next round" requires both players' consent (dual consent) so one
    // player can't unilaterally reset the board while the other is still
    // looking at the reveal - each side flips their own flag in
    // `ready_for_next`, and the server only advances once both are present.
