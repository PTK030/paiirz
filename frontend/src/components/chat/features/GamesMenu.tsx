...
export function GamesMenu({ onTrigger, onClose }: GamesMenuProps) {
  ...
  // Only the custom tab validates - the standard games (line ~85) have no
  // freeform fields, so there is nothing a user could submit invalidly.
  const handleSendThisOrThat = () => {
    ...
    onTrigger("this_or_that", {
      ...
      // The A/B option inputs are optional - falling back to generic labels
      // keeps the game playable even if the author only bothers typing a
      // question and leaves the options blank.
      options: [customOpt1.trim() || "Opcja A", customOpt2.trim() || "Opcja B"],
    });
    setCustomQ("");
    setCustomOpt1("");
    setCustomOpt2("");
    // Deliberately NOT resetting `tab` back to "standard" - closing and
    // reopening the menu for a second custom round should reopen on the tab
    // the user was already using, not force them to re-select it.
    onClose();
  };
