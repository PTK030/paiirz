...
function MessageBubbleImpl({
  ...
  return (
    ...
    >
      ...
      <div
        className={`relative flex items-center gap-2 group ${isMe ? "flex-row-reverse" : "flex-row"}`}
      >
        <AnimatePresence>
          {isActive &&
            (isExpanded ? (
              <motion.div
                ...
                className={`absolute bottom-full mb-2 flex flex-wrap gap-1 p-2 bg-zinc-900/90 border border-zinc-800/80 rounded-2xl backdrop-blur-xl shadow-2xl z-30 w-[240px] ${isMe ? "right-0" : "left-0"}`}
              >
              ...
              <motion.div
                ...
                exit={{ opacity: 0, y: 10 }}
                transition={{ duration: 0.12 }}
                className={`absolute bottom-full mb-2 flex items-center gap-1 p-1.5 bg-zinc-900/90 border border-zinc-800/80 rounded-2xl backdrop-blur-xl shadow-2xl z-30 ${isMe ? "right-0" : "left-0"}`}
              >
