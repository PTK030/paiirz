...
/**
 * @description Compact button pair shown in the message bar: "leave" ends
 * the conversation entirely (back to the setup/quick-start screen), while
 * "skip" (the arrow icon) calls the same `joinRoom` used for the initial
 * search - it leaves the current room and immediately re-queues for a new
 * stranger without returning to setup. Neither button is ever disabled
 * (unlike the mic/actions buttons in `MessageInputBar`) because both actions
 * are always safe to fire - leaving a room the server has already torn down,
 * or re-joining while already searching, are both handled idempotently by
 * `useRoom`.
 */
