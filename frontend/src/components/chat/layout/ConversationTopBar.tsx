...
import type { UseContactExchangeReturn } from "../../../hooks/ui/useContactExchange";
...
export function ConversationTopBar({
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      if (menuRef.current && !menuRef.current.contains(e.target as Node)) {
        if (contactMenuOpen || (exchangeState !== "idle" && exchangeState !== "exchanged")) {
          if (exchangeState === "input" || exchangeState === "waiting") {
  ...
  }, [contactMenuOpen, exchangeState, onToggleContactMenu, setExchangeState]);
      ...
      >
        ...
        <div className="relative">
          <button
            onClick={() => {
              if (exchangeState === "input" || exchangeState === "waiting") {
                // The exchange is in progress — a second click dismisses
                // the panel (lets the user cancel) and resets to idle.
                setExchangeState("idle");
                if (contactMenuOpen) onToggleContactMenu();
              } else {
                onToggleContactMenu();
              }
            }}
          ...
          >
            {exchangeState === "exchanged" ? (
              <span className="flex items-center gap-1.5">
                Kontakt <BsPersonVcard className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
              </span>
            ) : exchangeState === "waiting" ? (
              "Oczekiwanie..."
            ) : partnerWantsToExchange ? (
          ...
          <AnimatePresence>
            {/* Show whenever the menu is toggled open OR when the exchange
                is already in-flight (input/waiting/exchanged) so the user
                never loses the panel after clicking "Udostępnij dane". */}
            {(contactMenuOpen || exchangeState !== "idle") && (
              <motion.div
              ...
              >
                    ...
                    {partnerWantsToExchange ? (
                      <div className="flex gap-2">
                        <button
                          ...
                          className="flex-1 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold rounded-xl py-2 text-xs transition-all shadow-lg shadow-indigo-500/20"
                        >
                          Udostępnij
                        </button>
                        <button
                          onClick={() => {
                            exchange.setPartnerWantsToExchange(false);
                            if (contactMenuOpen) onToggleContactMenu();
                          }}
                ...
                {exchangeState === "exchanged" && partnerContact && (
                  <div className="flex flex-col gap-3">
                    ...
                    >
                      {copied ? "Skopiowano!" : "Skopiuj kontakt"}
                    </button>
                    <button
                      onClick={() => {
                        setExchangeState("idle");
                        // Clear partnerContact temporarily to allow a fresh exchange loop
                        // Though usually the backend just re-emits contact_exchanged.
                        // Setting to idle lets the user access the 'Udostępnij dane' button.
                      }}
                      className="w-full bg-transparent hover:bg-zinc-800/50 text-zinc-400 font-semibold rounded-xl py-1.5 text-[10px] transition-all"
                    >
                      Zaktualizuj / podaj inny kontakt
                    </button>
                  </div>
                )}
              </motion.div>
