...
const ChatWrapper: React.FC<ChatWrapperProps> = ({
  ...
  // Matches by class name (`.message-container` / `.reaction-picker`, applied
  // in `MessageBubble.tsx`) rather than a ref, because each open bubble's
  // reaction picker is rendered by `MessageBubble` itself - one per message in
  // the list - so a single shared ref can't target "whichever bubble is
  // currently active", but a class name present on every bubble/picker can.
  useEffect(() => {
