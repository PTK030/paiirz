...
export const Layout: React.FC<LayoutProps> = ({
  ...
  return (
    <div className={`bg-[#09090B] text-zinc-100 flex flex-col relative select-none font-sans overflow-hidden ${fullScreen ? "h-[100dvh]" : "min-h-full"} ${className}`}>
      ...
      {/* Main App Container */}
      <div className={`relative z-10 flex flex-col w-full h-full ${fullScreen ? "" : "px-6 sm:px-12 pt-6 sm:pt-8 overflow-y-auto scroll-container"}`}>
        {!hideHeader && <Header showBackLink={showBackLink} maxWidthClass={maxWidthClass} />}
        
        <main className={`flex-1 w-full mx-auto flex flex-col relative z-10 ${fullScreen ? "max-w-none" : `${maxWidthClass} mt-8 sm:mt-16 mb-16`}`}>
          {children}
        </main>
